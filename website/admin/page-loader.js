(function () {
  ...
  function patchImage(el, val, isBg, instant) {
  ...
  function applyData(D, instant) {
    ...
    document.querySelectorAll('[data-fw]').forEach(el => {
      ...
      if (val == null) return;
      if (el.tagName === 'IMG') {
        patchImage(el, val, false, instant);
      } else if (el.hasAttribute('data-fw-bg')) {
        patchImage(el, val, true, instant);
      } else {
  ...
  if (window.self !== window.top) {
    ...
    // double-click and is let through untouched, so real page buttons can
    // still be tested inside the preview.
    ...
    document.addEventListener('click', function (e) {
      ...
      const matchEl = fwEl || fwClickEl || sectionEl;
      const now = Date.now();
